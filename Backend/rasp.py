...
def displayRecognition(frame,scale,recognizedLocations, recognizedNames,recognizedIds,wpInfo,saveImage):
    for (top, right, bottom, left), name,id in zip(recognizedLocations, recognizedNames,recognizedIds):
            # Scale back up face locations since the frame we detected in was scaled to 1/4 size
            requisitos = list(wpInfo['requisitos'])            
            missingSkills=processMissingSkills(requisitos,id)   
...
def startProcessing(workplaceInfo):
    ...
    while True:
        ...
        if(len(recognizedNames)):
            preencheReconhecidos(workplaceInfo['N'],idsReconhecidos)
            saveImage,frame = displayRecognition(frame,scale,recognizedLocations,recognizedNames,idsReconhecidos,workplaceInfo,saveImage)
        else:
